...
describe('drawHeatmap', function () {
	describe('#tallyDomains', function () {
		it('should tally slice', function() {
			...
			tallyWasm = colsToObjs(tallyDomainsWasm(d, start, end, domains));
		...
		tallyWasm = colsToObjs(tallyDomainsWasm(d, start + 1, end - 1, domains));
		assert.deepEqual([
			{count: 0, sum: 0},
			{count: 2, sum: 2},
			{count: 2, sum: 10},
	...
	describe('#regionColor', function () {
		it('should color by first stop', function() {
			var domain = [0, 1],
				...
				color = regionColorLinearTest(domain, range, data, start, end);
