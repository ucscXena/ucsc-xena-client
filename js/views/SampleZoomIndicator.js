...
class SampleZoomIndicator extends PureComponent {
	...
	render() {
		var {data, column, addColumnAddHover, enableTransition, samples, zoom, onZoomOut} = this.props,
			{heatmap} = column,
				...
				<ColCard zoomCard={true}>
					...
					<div style={{position: 'relative'}}>
						<CanvasDrawing
							...
							heatmapData={heatmap}/>
						<ZoomCarriage addColumnAddHover={addColumnAddHover} enableTransition={enableTransition} samplesCount={samples.length} width={width} zoom={zoom} zoomOut={onZoomOut}/>
					</div>
				</ColCard>
			</div>);
	}
