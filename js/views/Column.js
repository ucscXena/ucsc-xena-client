...
import {IconMenu as RTIconMenu, MenuDivider, MenuItem} from 'react-toolbox/lib/menu';
...
function mutationMenu(props, {onMuPit, onShowIntrons, onSortVisible}) {
  ...
  if (data && !(_.isEmpty(data.refGene))) {
    mupitMenuItem = pos ?
      <TooltipMenuItem disabled={noMuPit} {...tooltipConfig("Only available for gene view")} caption={mupitItemName}/>
      : (noMuPit ? <TooltipMenuItem disabled={noMuPit} {...tooltipConfig("Only available for SNPs on hg19 and hg38")}
                                    caption={mupitItemName}/>
...
//		return 136;
  ...
  render() {
    var {
      ...
      menu = optionMenu(this.props, {
        onMode, onMuPit, onTumorMap, thisTumorMap, onShowIntrons, onSortVisible,
      ...
      <div style={{width: width, position: 'relative'}}>
        <ZoomOverlay geneHeight={geneHeight()} height={zoom.height}
          ...
          <ColCard colId={label}
            ...
            <div style={{
              cursor: selection ? 'none' : annotation ? `url(${crosshair}) 12 12, crosshair` : 'default',
              height: geneHeight()
            }}>
            ...
            <ResizeOverlay
              ...
              <div style={{position: 'relative'}} onMouseMove={this.onMouseMove} onMouseOut={this.onMouseOut}>
                <Crosshair frozen={!interactive || this.props.frozen} mousing={subColumnIndex.mousing}
                           geneHeight={geneHeight()} height={zoom.height} selection={selection}>
                  <DragSelect enabled={!wizardMode}
