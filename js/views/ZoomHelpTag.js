	...
	render() {
		...
		if (selection) {
			var {crosshair, direction, offset, overlay, zone, zoomTo} = selection,
				...
				tagInstruction = overlay.sstart === overlay.send ? 'Drag to zoom' : annotationZoom ? `Zoom to ${annotationPct < 1 ? `< 1` : annotationPct}%` : zoomTo.count === 1 ? `Zoom to row ${sampleRowFrom}` : `Zoom to rows ${sampleRowFrom} - ${sampleRowTo}`,
