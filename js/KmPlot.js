...
class LineGroup extends React.Component {
	shouldComponentUpdate(newProps) {
		return !_.isEqual(
			_.omit(newProps, "xScale", "yScale"),
			_.omit(this.props, "xScale", "yScale")
		);
	}
...
var formatPValue = v =>
	v == null ? String.fromCharCode(8709) : v.toPrecision(4);
	...
	render() {
		...
		return (
			<div className={kmStyle.warningContainer}>
				...
				{this.state.show ? (
					<WarningDialog onHide={this.close} header={header} body={body} />
				) : null}
			</div>
		);
	}
		...
		return (
			<div>
				<div className={kmStyle.PValueArea}>
					<div className={kmStyle.PValueP}>
						<i>P</i>-value = {formatPValue(pValue)}
					</div>
					{patientWarning ? (
						<WarningTrigger header="P value warning" body={patientWarning} />
					) : null}
				</div>
				<div>
...
class Legend extends PureComponent {
	render() {
		...
		let sets = _.zip(colors, curves, labels).map(set =>
			makeLegendKey(set, setActiveLabel, activeLabel)
		);

		return <div className={kmStyle.legend}>{sets}</div>;
	}
}
...
function makeGraph(
	...
	return (
		<div className={kmStyle.graph}>
			{svg(groups, setActiveLabel, activeLabel, {
	...
	return (
		<form>
			<div className={kmStyle.splits}>
				<label
					className={kmStyle.splitLabel}
					title="2 groups: top 50%, bottom 50%"
				>
					<input
						value={2}
						type="radio"
				...
				<label
					className={kmStyle.splitLabel}
					title="3 groups: top 33%, middle 33%, bottom 33%"
				>
					<input
						value={3}
						type="radio"
				...
				<label
					className={kmStyle.splitLabel}
					title="Quartiles: top 25%, bottom 25%"
				>
					<input
						value={-4}
						type="radio"
...
function makeDefinitions(
	...
	return (
		<div className={kmStyle.definitions} style={{ width: size.width }}>
			<PValue
				pValue={groups.pValue}
				logRank={groups.KM_stats}
...
class KmPlot extends PureComponent {
	...
	render() {
		let {
			km: { splits = 2, title, label, groups, cutoff, survivalType },
			survivalKeys,
			cohort,
			dims
		} = this.props,
		...
		let Content = _.isEmpty(groups) ? (
			...
			<div>
				<h3>
					Unfortunately, KM plot can not be made. There is no survival data
					overlapping column data.
        </h3>
			</div>
		) : (
					<div>
						<Button onClick={this.pdf} className={kmStyle.PDFButton}>
							<i className={`material-icons ${kmStyle.buttonIcon}`}>
								file_download
          </i>
