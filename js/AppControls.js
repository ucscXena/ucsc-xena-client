...
var BasicSearch = ({onTies, tiesEnabled, ...searchProps}) => (
	<>
		...
		{tiesEnabled && <IconButton onClick={onTies} className={compStyles.ties}><Icon fontSize='large'>toys</Icon></IconButton>}
	</>);
...
var TiesActions = ({onTies, onTiesColumn}) => (
	<>
		<Button color='secondary' onClick={onTiesColumn}>Create filtered column</Button>
		<IconButton edge='end' onClick={onTies}><Icon fontSize='large'>close</Icon></IconButton>
	</>);
...
export class AppControls extends PureComponent {
	...
	render() {
		...
		return (
			<AppBar color='default' component='div' position='relative'>
				<Box sx={sxControlBar}>
